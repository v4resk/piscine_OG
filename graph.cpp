...
//---------------------------------------------------------------------------------------------------------------------------------------
Graph::Graph(std::string nom_fichier, std::string fichier_ponderation) :  fichier(nom_fichier), fichier_pond(fichier_ponderation){
...
int Graph::get_orientation()
{
        return orientation;
}
std::vector<Sommet*>* Graph::get_tab_sommet()
{
        return &m_sommet;
}
std::vector<Arete*>* Graph::get_tab_arete()
{
        return &m_arete;
}
Sommet* Graph::getSommet_numeroN(int numero)
{
...
//------------------------------------------------------------------------
//----------------------SETTEUR-------------------------------------------
void Graph::ajouter_poid(int m_id,int poid)
{
...
void Graph::creer_svg(std::map<Sommet*,float> res_deg,std::map<Sommet*,float>res_prox,std::map<Sommet*,float>res_inter,std::map<Sommet*,float>res_propre)
{
...
//-----------------Auxilliaire--------------------------------
//------------------------------------------------------------
int Graph::search_poid_entre(Sommet* p,Sommet* succ){
        int poid = 0;
        for(auto a : m_arete)
        {
...
void Graph::ajouter_adjacents(int Si, int Sj)
{
...
int Graph::parcour_BFS(int nbr_s0)
{
        int ordre = 0;
...
void Graph::parcour_DFS(int m_id){
        ...
        for(auto it: s0->ObtenirAdjacents())
        {
                ...
                {
                        parcour_DFS(it->get_id());
                        it->setPred(s0);
                }
        }
}

void Graph::init_parcour_DFS(int m_id)
{
        set_all_unvisited();
        parcour_DFS(m_id);
...
void Graph::afficher_bfs(int m_id)
{
        ...
        for(it=m_sommet.begin(); it!=m_sommet.end(); ++it)
        {
                if(*it != s0)                 // si pas le sommet initiale
                {
                        ...
                        {
                                std::cout << (*it)->get_id() << "<---"; // On affiche le premier sommet de la liste ayant ete visite
                                Sommet* pred = (*it)->getPred();
                                if(pred != nullptr)
                                {

                                        while (pred != s0) {
                                                std::cout << pred->get_id() << "<--";
                                                pred = pred->getPred();
                                        }
                                }
                                std::cout << s0->get_id() <<  std::endl;
                        }
                }
        }
}

void Graph::afficher_dfs(int m_id)
{
        std::cout << std::endl << "Parcour DFS a partir du sommet "<< m_id << std::endl;
        afficher_bfs(m_id);
}

void Graph::supprimer_adja(int s, int s_sup)
{
...
//-------------------affichage----------------------------------
//--------------------------------------------------------------
void Graph::afficher(std::ostream& flux)
{
        if(!orientation)
